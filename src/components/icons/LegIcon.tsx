...
// Traced from the same reference SVG used in the original app (124 x 24.53 viewBox).
export default function LegIcon({ size = 64, color = 'currentColor', className }: LegIconProps) {
  const height = (size * 24.53) / 124;
  const maskId = 'legIconMask';

  return (
    <svg width={size} height={height} viewBox="0 0 124 24.53" className={className}>
      <defs>
        <mask id={maskId} x="-4.5" y="-19" width="128.5" height="64" maskUnits="userSpaceOnUse">
          <g>
            <rect x="27.5" y="-51" width="64" height="128" transform="translate(46.5 72.5) rotate(-90)" fill="#fff" />
      ...
      <g mask={`url(#${maskId})`} fill={color}>
        <rect x="45" y="-38.5" width="16" height="103" rx="4" ry="4" transform="translate(40 66) rotate(-90)" />
