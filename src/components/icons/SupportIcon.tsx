...
// Traced from the same reference SVG used in the original app — a
// horizontal truss/support beam (300 x 82 viewBox).
export default function SupportIcon({ size = 48, color = 'currentColor', className }: SupportIconProps) {
  const height = (size * 82) / 300;

  return (
    <svg width={size} height={height} viewBox="0 0 300 82" className={className}>
      <path
        fill={color}
